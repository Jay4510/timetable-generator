# CORRECTED USER-INPUT DRIVEN TIMETABLE ALGORITHM
# Based on actual system workflow - TT Incharge configures everything
# File: user_driven_timetable_algorithm.py
...
class UserConfigurableChromosome:
    ...
    def fitness(self, algorithm_instance):
        ...
        try:
            ...
            # Teacher preferences (user-configured)
...
class UserDrivenTimetableAlgorithm:
    ...
    def __init__(self, config_data, target_years=None):
        ...
        logger.info(f"User-driven algorithm initialized for years: {self.target_years}")
    ...
    def _generate_division_timetable(self, division, year_name):
        """Generate timetable for single division"""
        try:
            logger.info(f"Generating timetable for {year_name} Division {division.name}")
            
            # Create basic timetable sessions for this division
            sessions_created = self._create_basic_sessions(division, year_name)
            
            if sessions_created == 0:
                logger.warning(f"No sessions created for {year_name} Division {division.name}")
                return {'division': division.name, 'timetable': None, 'error': 'No sessions created'}
            
            logger.info(f"Created {sessions_created} sessions for {year_name} Division {division.name}")
            return {
    ...
    def _schedule_subject(self, subject, division, year_name, batches):
        ...
        try:
            ...
            logger.warning(f"Error scheduling subject {subject.name}: {e}")
        
        return genes
    
    def _schedule_project_work(self, subject, division, year_name, batches):
        """Schedule project work based on user configuration"""
        genes = []
        ...
        try:
            ...
            for i in range(len(available_slots) - 1):
                ...
                if (slot2 - slot1 == 1 and 
                    ...
                    # Schedule 2-hour lab
                    for batch in batches:
                        genes.append((subject.id, teacher_id, lab_id, slot1, batch['batch_number']))
                        genes.append((subject.id, teacher_id, lab_id, slot2, batch['batch_number']))
